...
class SentinelDataDownloader:
    ...
    def __init__(
        ...
        self.api = SentinelAPI(username, password, self._URL)
    ...
    def _determine_if_folder_zip_is_already_downloaded(self, folder_name):
        is_file_in_source_path = Path(
            self.download_directory_path, f"{folder_name}.zip"
        ).is_file()
    ...
    def _get_not_downloaded_uuid_isonline_dict(self, verbose=1):
        ...
        for _, row in self.products_df.iterrows():
            ...
            if (
                self._determine_if_folder_zip_is_already_downloaded(folder_name)
                is False
            ):
        ...
        if verbose >= 1:
            ...
            print(
                f"Number of online uuid: {numpy.array(list(uuid_dict.values())).sum()}"
            )
